...
const renderContent = () => {
  ...
  if (slotIdx === 2 || slotIdx === 4) {
    ...
    ? "<div class=\"card\" style=\"margin:0 0 12px 0; border:1px solid #e6eef2; background:#f9fcfd; padding:12px; border-radius:10px;\">" +
      ...
      "    <div style=\"font-weight:800;\">Consolidatie (merge)</div>" +
      "    <button id=\"openInputMergeBtn\" class=\"std-btn\" type=\"button\">Instellen</button>" +
      "  </div>" +
      "  <div style=\"margin-top:6px; font-size:12px; color:#607d8b;\">" +
      "    Voeg meerdere Input-kolommen samen tot 1 geconsolideerde Input (IN-id + export)." +
      "  </div>" +
      "</div>"
    : "";

  content.innerHTML = mergeTop + renderIoTab(data, slotIdx === 2);

  if (slotIdx === 2) {
    const mergeBtn = content.querySelector("#openInputMergeBtn");
    if (mergeBtn) {
      mergeBtn.addEventListener("click", () => {
        window.dispatchEvent(new CustomEvent("ssipoc:openMergeModal", { detail: { colIdx: editingSticky.colIdx, slotIdx: 2 } }));
      });
    }
  }
}
